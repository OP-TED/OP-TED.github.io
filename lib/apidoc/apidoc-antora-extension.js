                ...
                while (match = searchPattern.exec(referringFile.contents.toString())) {
                    ...
                    try {
                        if(apiSpec.startsWith('attachment$')) {
                            const attachmentPath = apiSpec.replace(/^attachment\$/, '')
                            basename = ospath.parse(attachmentPath).base
                            const apiSpecVirtualFile = componentVersionBucket.files.find(file => file.src.path == `modules/${module}/attachments/${attachmentPath}`)
                            const apiSpecFile = ospath.parse(ospath.resolve(ospath.format(tmpDir), 'specs', componentVersionBucket.name, componentVersionBucket.version, attachmentPath))
                        ...
                        addFile(createApidoc(module, apidocsDir, basename, apiSpec, generatorConfig, logger), componentVersionBucket.files, logger)
                        addFile(createIncludeFile(module, apidocsDir, basename, logger), componentVersionBucket.files, logger)
                    } catch (error) {
