        ...
        componentVersionBucket.files
            ...
            .forEach(referringFile => {
                ...
                while (match = searchPattern.exec(referringFile.contents.toString())) {
                    const apiSpec = new URL(match[1])

                    const basename = apiSpec.hostname.replace(/\./g, '-')
                    const startPath = ospath.parse(ospath.resolve(ospath.format(tmpDir), componentVersionBucket.name, componentVersionBucket.version))
