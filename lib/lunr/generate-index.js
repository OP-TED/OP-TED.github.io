...
function generateIndex (playbook, pages, contentCatalog, env) {
  ...
  const documents = pages
    ...
    .filter(({ page, $ }) => {
      ...
      const indexOnlyLatest = env.SEARCH_INDEX_VERSION &&
                              env.SEARCH_INDEX_VERSION === 'latest'
      if (indexOnlyLatest) {
    ...
    .map(({ page, $ }) => {
      ...
      return {
        ...
        componentTitle: contentCatalog.getComponentVersion(page.src.component, page.src.version).title,
  ...
  const languages = env.SEARCH_LANGS
    ? env.SEARCH_LANGS.split(',')
    : ['en']
